...
# Подсказка № 1
# Для генерации случайных чисел с плавающей запятой в определённом диапазоне
# используйте функцию random.uniform(a,b), где a и b — границы диапазона.В
# данном случае a = 5, b = 10.
# Подсказка № 2
# Для округления числа до двух знаков после запятой используйте функциюround().
# Например, round(random.uniform(5, 10),2).
# Подсказка № 3
# Для создания списка используйте генератор списков. Например,
# [round(random.uniform(5, 10), 2) for _ in range(20)] создаст список из
# 20 случайных вещественных чисел.
# Подсказка № 4
# Для определения победителя в каждой паре сравните соответствующие элементы
# двух списков. Используйте условный оператор внутри генератора списков:
